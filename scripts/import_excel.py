...
def upsert_to_local_postgres(cur, row):
    ...
    """, (
        ...
        str_val(row[COL["procedure_name"]]),
...
def main():
    ...
    print(f"Reading target sheet: {os.path.basename(filepath)}")
    rows = read_excel_data(filepath)
    print(f"Parsed {len(rows)} data rows from Excel.")

    print("Connecting to local PostgreSQL instance ('CalTest')...")
    try:
    ...
    for row in rows:
        ...
        try:
            ...
            conn.rollback()
            print(f"  ✕ Sync Failed for Serial {serial} ➔ {row_error}")
